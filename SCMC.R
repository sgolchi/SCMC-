...
adapt_seq = function(nu, t, N, sample, Wt) { 
  wt = c()
  for (i in 1:N) {
    term = constraint(sample[i,])
    cons1 = sum(pnorm(- term / nu, log = T))
    cons2 = sum(pnorm(- term / nuseq[t - 1], log = T))
    wt[i] = cons1 - cons2
  }
  Wt = Wt * exp(wt)
  Wt = Wt / sum(Wt)
...
logpost = function(sample, t) { 
  term = constraint(sample)
  return(sum(pnorm(- term / nuseq[t], log = T)))
}

# sampling at each time step
Gibbs = function(x, q, d, a, t = t, logpost.= logpost(), lpdent = lpdent) { 
  delta = rnorm(1,0,q[d])
  newx = x
  newx[d] = newx[d] + delta
  lpnum = logpost(newx, t)
  ratio = lpnum - lpdent
...
SCMC = function(constraint.= crescent(), N=1000, M=10, L=25, nuseq_T= 1e-3, lower=c(-1,-1), upper=c(1,1), qt = 1) {
  ...
  for (i in 1:N) lpdent[i, 1] = logpost(samplet[i, t,], t)
  Wt[,1] = rep(1 / N, N)
  repeat {
    t = t+1
    ...
    newWt = Wt[, t - 1]
    nuseq[t] = ifelse(adapt_seq(nuseq_T, t = t, N = N, sample = newsample, Wt = newWt) > 0, nu_T, uniroot(adapt_seq, interval = c(ifelse(t < 3, min(.5, nuseq_0[t]), nu_T), ifelse(t != 2, nuseq[t-1], 1e5)), t = t, N = N, sample = newsample, Wt = newWt)$root)
    wt = c()
    for (i in 1:N) {
    ...
    for(j in 1:M) {
      for (i in 1:N) {
        for (d in 1:D) {
          out = Gibbs(newsample[i,], q, d, a[t, d], t = t, logpost.= logpost(), lpdent = newlpdent[i])
          newsample[i,] = out$x
          a[t,d] = out$a
